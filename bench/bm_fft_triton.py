...
class benchmark_fft(gr.top_block):
    ...
    def __init__(self, args):
        ...
        for i in range(num_blocks):
            context = {"max_batch_size": args.max_batch_size, "fft_size": fft_size, "device": "cpu", "shift": False, "forward": True}
            torchdsp.DynamicModels.render_model('fft_cc', '/tmp/rendered_triton_models/bm_fft_model', context, 'localhost:8000')

            blks.append(
                torchdsp.triton_block("bm_fft_model", args.max_batch_size, 'localhost:8000', [8*fft_size], [8*fft_size])
            )
